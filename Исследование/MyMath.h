...
void discriminant() {
	...
	float a, b, c;
	cout << "¬ведите a,b,c"<< endl;
	cin >> a; cin >> b; cin >> c;
	float d = sqrt(b*b - 4 * a * c);
	float root1 = (-b + d) / (2 * a);
	float root2 = (-b - d) / (2 * a);
	cout << "ѕервый корень " << root1 << endl;
	cout << "¬торой корень " << root2 << endl;
	system("pause");
}
...
void ProgramistAndWebProgramistDay() {
	...
	for (int i = 1; i <= 101; i++) 
	{
		...
		if ((year % 4 != 0) || (year % 100 == 0 && year % 400 != 0)) {
			...
			if (denP == 0)denP = 7;
			cout <<"ƒень программиста "<< denP << endl;
			//////////////////////
			for (int i = 1; i < 94; i++) {
				if (denWP == 7)denWP = 0;
				denWP++;
			...
			if (denWP == 0)denWP = 7;
			cout << "ƒень веб-программиста " << denWP << endl;
			//////////////////////
...
void SumOfSinuses() {
	...
	for (int i = 0; i < 100; i++) {                                        //100 входов в цикл дл€ перебора
		...
		for (int i=0;i<20; i++) {													//смешение влево на символ
			for (int k = 0; k < 100; k++) {										//ѕеребор в одной €чейке			
				s[0][19 - l] = p[0][k];
				for (int i = 0; i < 20; i++) {									//счЄт суммы 
					Sum = Sum + s[0][i];
				}
				if (Sum == 200) {
					for (int i = 0; i < 20; i++) {
						SinSum = SinSum + s[1][i];								//счЄт синуса суммы
					}
					if (SinSum > MaxSinSum) {
